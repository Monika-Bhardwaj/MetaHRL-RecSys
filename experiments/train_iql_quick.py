...
def main():
    # ----- Quick test mode: use only 1M rows -----
    ...
    loader = DataLoader(
        dataset,
        batch_size=128,  # smaller batch for quick runs
        shuffle=True,
        num_workers=4,
    ...
    # ----- Models -----
    q_net = QNetwork(num_items, embed_dim).to(device)
    v_net = ValueNetwork(num_items, embed_dim).to(device)
    pi_net = PolicyNetwork(num_items, embed_dim).to(device)
    ...
    for step, batch in enumerate(loader):
        ...
        with torch.no_grad():
            target_v = v_net(next_state)
            q_target = reward + gamma * target_v * (1 - done)
